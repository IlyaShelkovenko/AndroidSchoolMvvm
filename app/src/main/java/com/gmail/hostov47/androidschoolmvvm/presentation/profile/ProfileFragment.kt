...
class ProfileFragment
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if(binding.ivAvatar.isVisible
            ...
            setImageFromContent(imageUriString, requireContext())
        }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == RESULT_CHOOSE_IMAGE && resultCode == RESULT_OK && null != data) {
            ...
            setImageFromContent(selectedImage.toString(), requireContext())
        }
    }
        ...
        if (requestCode == REQUEST_CODE) {
            if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                if (ActivityCompat.shouldShowRequestPermissionRationale(requireActivity(), permissions[0])) {
                    Toast.makeText(requireContext(), "I need this permission", Toast.LENGTH_LONG).show()
                }
                checkPermissions()
            }
    ...
    private fun setImageFromContent(imageUriString: String, context: Context) {
